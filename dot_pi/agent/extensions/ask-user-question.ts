...
export default function askUserQuestion(pi: ExtensionAPI) {
	pi.registerTool({
		...
		async execute(_toolCallId, params, signal, _onUpdate, ctx) {
			...
			// RPC has dialog support but cannot render custom terminal components.
			if (ctx.mode === "rpc" && mode !== "text") {
				return unavailableResult(params.question, mode,
					"Choice dialogs require Pi's terminal UI. In RPC, ask a free-form question without options instead.", context);
			}

			return withUILock(async () => {
				// The call may have been cancelled while another question held the lock.
