...
export default function omWorker(pi: ExtensionAPI): void {
	...
	trackWorkerCost(pi);
	// A zero process exit is not proof of a successful provider response.
	pi.on("agent_end", (event: any) => {
		const path = process.env.OM_STATUS_PATH;
		if (!path) return;
		const message = [...(event.messages ?? [])].reverse().find((m: any) => m?.role === "assistant");
		atomicWrite(path, JSON.stringify({ ok: message?.stopReason === "stop" }));
	});
