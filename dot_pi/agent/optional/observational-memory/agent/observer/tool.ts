...
export function registerObserverTool(pi: ExtensionAPI, resultPath: string): void {
	...
	function flush(): void {
		writeObserverResult(resultPath, { observations: accumulated, recorded });
	}
	...
	pi.registerTool({
		...
		async execute(_id: string, params: RecordObservationsInput, _signal: AbortSignal | undefined, _onUpdate: unknown, _ctx: ExtensionContext) {
			...
			if (accumulated.length > 1000 || Buffer.byteLength(JSON.stringify(accumulated)) > 200 * 1024) {
				throw new Error("Observer output exceeded its limit; shorten the observations");
			}
			flush();
