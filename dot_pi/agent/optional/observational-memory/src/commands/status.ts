...
export function registerStatusCommand(pi: ExtensionAPI, runtime: Runtime): void {
	pi.registerCommand("om:status", {
		...
		handler: async (_args: string, ctx: any) => {
			...
			const lines = [
				`om status`,
				`  observer model: ${runtime.config.models.observer.provider}/${runtime.config.models.observer.id} (${runtime.config.models.observer.thinking ?? "default"})`,
				`  consolidator model: ${runtime.config.models.consolidator.provider}/${runtime.config.models.consolidator.id} (${runtime.config.models.consolidator.thinking ?? "default"})`,
