...
import { hasCompleteCoverage } from "../ledger/coverage.js";
import {
		...
		try {
			...
			if (!skip) {
				...
				try {
					...
					if (onAbort) event.signal?.removeEventListener("abort", onAbort);
				}
			...
			if (event.signal?.aborted || !runtime.enabled || runtime.epoch !== epoch) return { cancel: true };
			const snapped = snap.firstKeptId;
			if (!hasCompleteCoverage(branch, snapped)) {
				if (hasUI) ctx.ui.notify("om: history coverage is incomplete; using Pi's normal compaction instead", "warning");
				return undefined;
			}
			...
			if (!summary.trim() || Buffer.byteLength(summary) > 64 * 1024) {
				if (hasUI) ctx.ui.notify("om: memory summary is empty or exceeds 64 KiB; using Pi's normal compaction", "warning");
				return undefined;
			}
