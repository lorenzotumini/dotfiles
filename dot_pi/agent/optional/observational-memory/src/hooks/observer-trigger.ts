...
import { readObserverResult, readWorkerCost, runCostPath, runResultPath, runStatusPath } from "../spawn/runs.js";
import { requireWorkerSuccess } from "../spawn/receipt.js";
import { contiguousCoverageMarker } from "../ledger/coverage.js";
...
/** Only a contiguous covered/in-flight prefix can advance dispatch. */
function effectiveWatermarkId(runtime: Runtime, branch: Entry[]): string | undefined {
	return contiguousCoverageMarker(branch, runtime.observersInFlight.values());
}
...
async function dispatchObserver(
	...
	const sourceEntryIds = slice.entries.map(entry => entry.id);
	runtime.observersInFlight.set(runId, { controller, coversUpToId, sourceEntryIds });
	...
	try {
		...
		if (!result.recorded) throw new Error("Observer never called record_observations; retaining raw history");
		const branch = ctx.sessionManager.getBranch();
		const currentIds = new Set(branch.map(entry => entry.id));
		if (!sourceEntryIds.every(id => currentIds.has(id))) return;
		...
		pi.appendEntry(OM_OBSERVATIONS_RECORDED, { observations, coversUpToId, sourceEntryIds });
