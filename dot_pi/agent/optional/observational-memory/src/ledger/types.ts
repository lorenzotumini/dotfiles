...
export type ObservationsRecordedEntryData = {
	...
	coversUpToId: string;
	/** Orchestrator-owned chunk membership, including explicitly empty chunks. */
...
export function isObservationsRecordedData(value: unknown): value is ObservationsRecordedEntryData {
	...
	return (
		...
		(value.observations.length > 0 || isNonEmptyStringArray(value.sourceEntryIds)) &&
		value.observations.every(isObservation) &&
		isNonEmptyString(value.coversUpToId) &&
		(value.sourceEntryIds === undefined || (isNonEmptyStringArray(value.sourceEntryIds) && value.sourceEntryIds.at(-1) === value.coversUpToId))
	);
}
