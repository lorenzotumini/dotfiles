...
import { resolveWithinMemory } from "../memory/paths.js";
import { fileURLToPath } from "node:url";
import { dirname, join, resolve } from "node:path";
import type { ConfiguredModel } from "../config.js";
import { runCostPath, runResultPath, runStatusPath } from "./runs.js";
...
export function resolvePiBinary(): { command: string; baseArgs: string[] } {
	// Never mistake an SDK host or test runner for the Pi CLI.
	const packageDir = (Pi as unknown as { getPackageDir?: () => string }).getPackageDir?.();
	const cli = packageDir ? join(packageDir, "dist", "cli.js") : undefined;
	return cli && existsSync(cli) ? { command: process.execPath, baseArgs: [cli] } : { command: "pi", baseArgs: [] };
}
...
export function spawnWorker(opts: {
	...
	if (opts.signal?.aborted) return Promise.resolve({ code: 1, signal: null, stderr: "worker cancelled before launch" });
	if (!resolveWithinMemory(opts.cwd, ".")) throw new Error("worker directory contains a symlink");
	const [command, ...rest] = opts.argv;
	mkdirSync(opts.cwd, { recursive: true, mode: 0o700 });
	// Keep transcript chunks out of OS-visible argv and avoid E2BIG. Pi still
	// records the expanded @file as its initial user message.
	if (opts.env.OM_WORKER && opts.env.OM_RESULT_PATH) {
		const p = rest.indexOf("-p");
		if (p >= 0 && rest[p + 1]) {
			const promptPath = `${opts.env.OM_RESULT_PATH}.prompt.md`;
			if (!resolveWithinMemory(opts.cwd, promptPath)) throw new Error("invalid worker prompt path");
			mkdirSync(dirname(promptPath), { recursive: true, mode: 0o700 });
			writeFileSync(promptPath, rest[p + 1], { mode: 0o600, flag: "wx" });
			rest[p + 1] = `@${promptPath}`;
		}
	}
	return new Promise<WorkerExit>((resolvePromise) => {
		...
		let stderr = Buffer.alloc(0), settled = false, stopping = false;
		let escalation: ReturnType<typeof setTimeout> | undefined;
		const stop = () => {
			if (settled || stopping) return;
			stopping = true;
			proc.kill("SIGTERM");
			escalation = setTimeout(() => { if (!settled) proc.kill("SIGKILL"); }, 3000);
			escalation.unref();
		};
		const deadline = setTimeout(stop, opts.timeoutMs ?? 300_000);
		deadline.unref();
		function finish(code: number | null, signal: NodeJS.Signals | null, fallback = "") {
			if (settled) return;
			settled = true;
			clearTimeout(deadline); if (escalation) clearTimeout(escalation);
			opts.signal?.removeEventListener("abort", stop);
			const text = stderr.toString("utf8").slice(0, 60_000);
			resolvePromise({ code: stopping ? 1 : code, signal, stderr: text || fallback || (stopping ? "worker cancelled or timed out" : "") });
		}
		proc.stderr?.on("data", (d: Buffer) => { stderr = Buffer.concat([stderr, d]).subarray(-16_384); });
		proc.on("error", () => finish(1, null, "worker spawn failed"));
...
export function buildWorkerEnv(role: "observer" | "consolidator", opts: ObserverLaunchEnv): NodeJS.ProcessEnv {
	return {
		...
		OM_STATUS_PATH: runStatusPath(opts.memoryRoot, opts.runId),
		PI_OFFLINE: "1", // skip startup update/telemetry checks, not model requests
