...
def _extract_text(content) -> str:
    return S._extract_text(content)
...
def _render_markdown(sess_prompts, total_prompts: int) -> None:
    ...
    proj_order = sorted(by_proj.keys(),
                        key=lambda k: max((s.started_at.timestamp() for s, _ in by_proj[k]
                                           if s.started_at), default=float('-inf')),
                        reverse=True)
    ...
    for proj in proj_order:
        ...
        for s, prompts in sorted(by_proj[proj],
                                 key=lambda x: x[0].started_at.timestamp() if x[0].started_at else float('-inf'),
                                 reverse=True):
