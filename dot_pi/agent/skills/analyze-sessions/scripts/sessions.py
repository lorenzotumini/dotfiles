...
SESSIONS_ROOT = Path(os.environ.get("PI_CODING_AGENT_SESSION_DIR") or
                     str(Path(os.environ.get("PI_CODING_AGENT_DIR", "~/.pi/agent")).expanduser() / "sessions")).expanduser()
    ...
    with f:
        for line in f:
            ...
            if not isinstance(rec, dict):
                continue
            t = rec.get("type")
            usage, model_key = record_usage(rec)
            if usage is not None:
                before = s.cost_total
                _accumulate_usage(s, usage)
                s.cost_by_model[model_key] = s.cost_by_model.get(model_key, 0.0) + s.cost_total - before
            if isinstance(rec.get("timestamp"), str):
                try:
                    stamp = ts_from_iso(rec["timestamp"])
                    s.last_at = max(s.last_at, stamp) if s.last_at else stamp
                except ValueError:
                    pass
            if t == "session":
                s.id = rec.get("id", "")
                ...
                mts = m.get("timestamp")
                if isinstance(mts, (int, float)) and math.isfinite(mts):
                    try:
                        stamp = ts_from_ms(int(mts))
                        s.last_at = max(s.last_at, stamp) if s.last_at else stamp
...
def record_usage(rec: dict):
    """Count only explicit usage, never embedded retainedTail/history copies.

    Tool and summary usage lack reliable model provenance in Pi's schema.
    Keep separate unknown buckets instead of guessing the current model.
    """
    if rec.get("type") == "message":
        m = rec.get("message")
        if not isinstance(m, dict) or m.get("role") not in ("assistant", "toolResult"):
            return None, None
        key = (f"{m.get('provider') or '?'}/{m.get('model') or '?'}"
               if m.get("role") == "assistant" else "[tool usage: model unknown]")
        usage = m.get("usage")
    elif rec.get("type") in ("compaction", "branch_summary"):
        key = f"[{rec['type']} usage: model unknown]"
        usage = rec.get("usage")
    else:
        return None, None
    return (usage, key) if isinstance(usage, dict) else (None, None)
...
def _accumulate_usage(s: SessionSummary, usage) -> None:
    ...
    components = [number(cost.get(k)) for k in ("input", "output", "cacheRead", "cacheWrite")]
    s.cost_total += number(cost["total"]) if cost.get("total") is not None else sum(components)
    s.cost_input += components[0]
    s.cost_output += components[1]
    s.cost_cache_read += components[2]
    s.cost_cache_write += components[3]
    s.tok_input += int(number(usage.get("input")))
    s.tok_output += int(number(usage.get("output")))
    s.tok_cache_read += int(number(usage.get("cacheRead")))
    s.tok_cache_write += int(number(usage.get("cacheWrite")))
    ...
    def matches(self, s: SessionSummary) -> bool:
        if self.session_id:
            if not (s.id == self.session_id or s.id.startswith(self.session_id)):
                return False
        if (self.since or self.until) and s.started_at is None:
            return False
        if self.since and s.started_at < self.since:
            return False
        if self.until and s.started_at > self.until:
            return False
        if self.cwd_substrs:
...
def cli_date(value: str) -> str:
    try:
        parse_date(value)
    except (ValueError, OverflowError):
        raise argparse.ArgumentTypeError("expected a valid date, ISO datetime, or relative duration")
    return value
...
def add_filter_args(p: argparse.ArgumentParser, *, subagents_default: bool) -> None:
    ...
    """
    p.add_argument("--since", type=cli_date, metavar="WHEN", help="Lower bound: YYYY-MM-DD, ISO datetime, or relative (7d/2w/3h/30m).")
    p.add_argument("--until", type=cli_date, metavar="WHEN", help="Upper bound, same formats as --since.")
    ...
    p.add_argument("--provider", metavar="ID", help="Exact provider ID, including custom providers.")
    ...
    p.add_argument("--limit", type=positive_int, help="Cap sessions scanned (cost.py: displayed groups instead).")
...
def filters_from_args(args: argparse.Namespace, *, subagents_default: bool) -> Filters:
    ...
    if getattr(args, "until", None):
        ...
        if re.fullmatch(r"\d{4}-\d{2}-\d{2}", args.until.strip()):
            f.until += timedelta(days=1, microseconds=-1)
