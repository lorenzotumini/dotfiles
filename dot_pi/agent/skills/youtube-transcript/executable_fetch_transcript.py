...
def get_metadata(url):
    ...
    try:
        ...
        print(f"Error executing yt-dlp: {e.stderr[-2000:]}", file=sys.stderr)
        sys.exit(1)
...
def download_subtitle(url, lang, is_auto, outdir):
    ...
    try:
        ...
        print(f"Error downloading subtitles via yt-dlp: {e.stderr[-2000:]}", file=sys.stderr)
        sys.exit(1)
    except (subprocess.TimeoutExpired, FileNotFoundError) as e:
        print(f"Subtitle download failed: {type(e).__name__}", file=sys.stderr)
        sys.exit(1)
